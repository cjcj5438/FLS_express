...
function params(b,p){
    if(!Object.prototype.hasOwnProperty.call(p,'params')){return dbs.error(b,-1,'查询参数不存在');}
    let ps='';
    ...
    if(!db.exec(sql)){return dbs.error(b,100,db.lastErrorText());}
    b.data={};
    while(db.next()){
...
function save(b,p){
    ...
    const sql='INSERT INTO system_config(key,value) VALUES '+ps.join(',')+
